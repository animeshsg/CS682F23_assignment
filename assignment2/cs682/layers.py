...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache=(x,x_mean,x_var,x_std,x_hat,gamma)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
        ...
        #######################################################################
        running_std=(running_var+eps)**0.5
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta=np.sum(dout,axis=0)
    dldxhat=dout*gamma
    dldxhat_sum=np.sum(dldxhat,axis=0)

    dx=(M*dldxhat - dldxhat_sum - np.sum(dldxhat*x_norm,axis=0)*x_norm) / (x_std*M)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out=np.zeros((N,F,Hout,Wout)) #output dummy

    #padding init
    pwidth=((0,0),(0,0),(pad,pad),(pad,pad))
    xpad = np.pad(x,pwidth,'constant')
    #print(xpad.shape)
    N,C,Hpad,Wpad=xpad.shape

    for n in range(N):
      for f in range(F):
        for i in range(0,Hpad-HH+1,stride):
          for j in range(0,Wpad-WW+1,stride):
            out[n,f,i//stride,j//stride]=np.sum(xpad[n,:,i:i+HH,j:j+WW]*w[f])+b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for f in range(F):
            for i in range(0,H_pad-HH+1,stride):
                for j in range(0,W_pad-WW+1,stride):
                    ...
                    dw[f]+=dout[n,f,i//stride,j//stride]*xpad[n,:,i:i+HH,j:j+WW]

                    dxpad[n,:,i:i+HH,j:j+WW]+=dout[n,f,i//stride,j//stride]*w[f]
                    
                    
    dx=dxpad[:,:,pad:-pad,pad:-pad] #undo padding
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_out=1+(H-pH)//stride
    W_out=1+(W-pW)//stride
    out=np.zeros([N,C,H_out,W_out]) #output dummy
...
def spatial_groupnorm_backward(dout, cache): #fix
    ...
    dx = (1.0/Nxn)*1.0/x_std *(Nxn* dldxhat-dnorm_sum-dldxhat*np.sum(dldxhat*x_norm,axis=0))
